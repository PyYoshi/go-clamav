...
`main` はリポジトリルールセットで保護されています: 変更は全CIチェックが
greenのプルリクエスト経由でのみ取り込まれ、全コミットに検証済み署名が必須で、
マージはマージコミット方式のみです(署名を保持するためsquash/rebaseマージは
無効化)。リリースタグ(`v*`)の削除・付け替えはできません。

開発ワークフローは [CONTRIBUTING.md](CONTRIBUTING.md) を、人間・AIを問わず
全コントリビューターが従う規約は [AGENTS.md](AGENTS.md) を参照してください。
クローン後に一度 `make setup` を実行するとリポジトリのgitフックが有効になります。
